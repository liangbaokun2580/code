...
class ModeManager:
    ...
    def get_mode_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'mode_descriptions': {
                'if_else': '基于关键词匹配的 if...else 模式，支持预定义响应和 Function Call',
        ...
        if target_mode == APIMode.OLLAMA:
            ...
            try:
                ...
                if not ollama_client.is_available():
                    ...
                    result['message'] = f"Ollama 服务不可用，请检查 {self.ollama_config['base_url']} 是否正常运行"
                else:
                    models = ollama_client.list_models()
                    if not models:
                        result['warnings'].append("未找到可用的 Ollama 模型，请先下载模型")
                    elif self.ollama_config['default_model'] not in [m.get('name', '') for m in models]:
                        result['warnings'].append(f"默认模型 {self.ollama_config['default_model']} 不存在")
            except Exception as e:
                result['can_switch'] = False
